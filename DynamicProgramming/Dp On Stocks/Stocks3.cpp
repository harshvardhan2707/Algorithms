...
int finalAns(vector<int> &prices, int i, int n,bool canBuy,int no,vector<vector<vector<int>>> &dp){
    if(i==n)return 0;
    if(no==2)return 0;
    if(dp[i][canBuy][no]!=-1)return dp[i][canBuy][no];
    if(canBuy){
        int buys=-prices[i]+finalAns(prices,i+1,n,false,no,dp);
        int doesntBuy=finalAns(prices,i+1,n,true,no,dp);
        return dp[i][canBuy][no]=max(buys,doesntBuy);
    }
    int sells=prices[i]+finalAns(prices,i+1,n,true,no+1,dp);
    int doesntSell=finalAns(prices,i+1,n,false,no,dp);
...
int SpaceOptimization(vector<int>& prices, int n){
...
//     return finalAns(prices,0,n,true,0,dp);
//     return Tabulation(prices,n);
    return SpaceOptimization(prices,n);
}